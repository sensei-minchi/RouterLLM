...
4) Assign score for each prompt :- score = a*quality_score - b*latency - c*cost
5) Encode each prompt, and then train a model with input [embedding, length_of_prompt] to give [score] as the output
